...
FunOrder
=========
The Functional Order (FunOrder) tool - Identification of essential biosynthetic genes through computational molecular co-evolution – leverages the hypothesis of co-evolutionary linked genes for this task. As co-localization and co-expression have been used for the detection of BGCs, we demonstrate that co-evolution can be promoted to determine which genes to choose for heterologous expression in Ascomycete. Due to the flexibility and adaptability of the core program to any given protein database, it is possible to integrate other separate databases for the analysis of plant BGCs, early diverging fungi BGCs, bacterial BGCs, archaeal BGCs or Basidiomycete BGCs. We aim to develop further databases to be integrated into our robust FunOrder tool. As a future perspective, given the increase in computational resources, FunOrder might be used for the analysis of co-evolution on a whole proteome, enabling the genome wide detection of evolutionary linked genes. Making a first step in tackling the issue of fungal genes involved in SM production dispersed over the genome and thereby undetectable by conventional software tools searching for clustering genes.
...
Vignolle, G. A.; Mach, R. L.; Mach-Aigner, A. R.; Derntl, C. The Functional Order (FunOrder) tool: Identification of essential biosynthetic genes through computational molecular co-evolution. *Nucleic Acids Research*, **2020**. [DOI: ](https://doi.org/)
...
The top 20 sequences from the database search from the manually curated fungal proteome database are extracted and concatenated with the query sequence in one file. A customized Perl script removes the duplicate entries in the file if any were introduced. Using emma a multiple sequence alignment of the protein sequences based on the ClustalW algorithm is calculated and a dendrogram computed. Using RAxML (Randomized Axelerated Maximum Likelihood) first 100 rapid Bootstraps are performed and then a search for the best-scoring maximum likelihood (ML) tree. These phylogenetic trees are computed for each gene in the BGC using the LG amino acid substitution model. Furthermore a standard ascertainment bias correction by Paul O. Lewis is performed. Then after inferring the ML trees from the multiple sequence alignment the strict distance and speciation distance between all inferred trees is computed using the TreeKO algorithm. The **strict distance** is computed without taking evolutionary exceptions into account, such as duplication events or different species content of the two compared trees. The topology of a pair of trees is compared, a distance of 0 indicates topologically identical trees. The speciation distance or evolutionary distance is computed to compare the topologies of a pair of trees and enumerate their shared evolution of their shared species. The **evolutionary distance** can correct for duplication and loss events and infers shared "speciation history" based solely on topology without considering branch lengths. In case of different gene phylogenies, where the aim is the detection of co-evolved genes, the comparison of the two distance measures solidifies the inference of linked genes. This comparison is made possible through a custom BASH script and a special identifier introduced in the fungal proteome database. All pairwise strict and evolutionary distances are saved as matrices respectively. The matrices are used as input for an R script for visualization and further analysis of the distances with various packages. The strict and evolutionary distances are summed up to a third combined distance measure. 
